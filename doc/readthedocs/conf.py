...
version = re.sub(r'[\s\S]*AC_INIT.*?([\d\.]+)[\s\S]*', r'\1', open('../../configure.ac').read())
release = version
...
# breathe_projects = { 'COFFI': './xml' }
# breathe_default_project = 'COFFI'

def run_doxygen(app):
    '''Run the doxygen command'''
    try:
        run_mod(app)
        # Note: last time I checked, Read the Docs was running Doxygen version 1.8.13 (package version in ubuntu bionic)
        retcode = subprocess.call('doxygen', shell = True, cwd = '..')
        if retcode < 0:
